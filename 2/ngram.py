...
class LanguageModel(object):
    ...
    def ngram_prob(self, ngram):
        ...
        # TODO: TASK 3
        return ngram_count + sys.float_info.epsilon \
            / (context_count + sys.float_info.epsilon + V)
    ...
    def sentence_logprob(self, sentence):
        ...
        for i in range(l - self.ngram_size + 1):
            prob = self.ngram_prob(tuple(sentence[i: i + self.ngram_size]))
            logprob += np.log(prob)
        return logprob
...
if __name__ == '__main__':
    ...
    logprob1, logprob2 = np.array(basic_lm.log_prob(test1)), \
                         np.array(basic_lm.log_prob(test2))
